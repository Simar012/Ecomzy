...
const Header = () => {
  ...
  return (
    ...
    <header className="bg-black fixed top-0 py-3.5 w-full max-w-full overflow-x-hidden z-10">

      {/* <!-- navbar container --> */}
      <div className="w-full max-w-full px-1 sm:px-4 m-auto flex items-center gap-3 flex-wrap">

        {/* logo & search */}
        <div className="flex items-center flex-1 min-w-0">
          <Link className="h-9 mr-1 sm:mr-4" to="/">
            <img draggable="false" className="h-full w-full object-contain" src={logo} alt="Ecomzy Logo" />
          </Link>
        ...
        <div className="flex items-center gap-1 sm:gap-7">
          {isAuthenticated === false ? (
            <Link
              to="/login"
              className="px-3 sm:px-9 py-0.5 text-primary-blue bg-white border font-medium rounded-sm cursor-pointer text-center whitespace-nowrap"
            >
              Login
            </Link>
          ) : (
